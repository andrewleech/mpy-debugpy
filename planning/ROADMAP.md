...
- **STORY-6.7 — DAP interleaved with the REPL on a single UART**
  ...
  - description: the no-network answer for boards with one UART, which is most
    ...
    either escape it or route program output through the DAP `output` event
    instead, which `DebugSession.send_output` already emits.
  - acceptance criteria:
    ...
    - [ ] whichever of escaping / `output`-event routing is chosen, the
