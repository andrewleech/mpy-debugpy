...
- **The separate serial DAP path is removed (2026-08-20).** D9. It was never available
  ...
  it always cost a build, then a `boot.py` edit and a re-enumeration on top, for a channel
  mpremote had to bridge and frame itself. With `network.USBD_NCM` upstream, a custom build
  buys the mainline network transport instead, and a board as it ships already has the REPL
  stream. **STORY-6.1 is superseded**: what it delivered is deleted, and the questions it
  answered (Q12, D3, D7) are now history rather than live constraints. The one thing lost
  with it is measured rather than assumed - 81.7-108.8 kB/s against the REPL stream's
  81.5-81.7 - which is headroom, not capability. `tests/hil/` drops the two-arrangement
  dance that existed only to serve it.
- **PARKED 2026-08-15: the WiFi stall is not this project's to fix, and the hardware suite now retries once against its signature alone.** The fault lives below MicroPython - frames lost between the access point and lwIP's input, with every layer above excluded by measurement - so there is nothing here to change, and it was blocking roadmap work at about one hardware run in twenty. The mitigation is deliberately narrow: `tests/hil/conftest.py` marks hardware scenarios `flaky(reruns=1, only_rerun=_PARKED_STALL_SIGNATURES)`, and those two regexes match only "the board answered earlier requests on this connection and then stopped" - `no response to <cmd> (seq N) within` and `no initialize response from`. Anything else still fails on its first attempt, one retry is the bound (two would turn a one-in-twenty fault into a one-in-four-hundred one, which is indistinguishable from having fixed it), and a retried scenario is *recorded as retried* in the results record with a note saying why, so the rate the risk row tracks stays visible instead of being converted into a slower green run. `tests/test_hil_retry_policy.py` pins both halves on the host, against failure texts copied verbatim from real runs of each kind. Everything known is in `20260814_wifi_stall_investigation_handover.md`; unparking it means either a report upstream or one of the three instruments listed there (cyw43 driver logging, a monitor-mode capture, a second access point).
- **The wedged connection loses its frames below lwIP, measured with lwIP's own counters (2026-08-14).** The addendum to `20260813_wifi_powersave_tcp_stall.md`. An instrumented build of the current pin - `LWIP_STATS` on plus a `lwip.print_stats()` returning the proto counters as a dict, because lwIP's `stats_display()` formats every counter with `%hu` and MicroPython's printf prints the format string - was flashed to the bench board, measured, and the original image restored. **The wedge involves no lwIP drop at all**: across an outbound-connection wedge `tcp.drop` did not move, `chkerr`, `lenerr`, `proterr` and `err` stayed at zero, and all 22 unicast pings sent during the stall were received and answered (`icmp.recv` and `icmp.xmit` both +22). The capture agrees: the board's last acknowledgement is 420, and `420:507` is retransmitted eight times over 34 s without an ACK, a RST or a counter moving. So the frames are lost between the access point and lwIP's input, and everything above that is now excluded by measurement rather than by argument - not debugpy, not lwIP's TCP, not MicroPython's accept path (an outbound connection, which never installs `_lwip_tcp_recv_unaccepted`, wedges identically), and not the socket-timeout alternation. The accept arm does move `tcp.drop`, and with the other error counters at zero that can only be `tcp_in.c:432`'s refused-data path, but it is not the wedge: the outbound arm wedges with the counter untouched, and the likeliest reading is stale pcbs from connections queued while the one-connection-at-a-time harness was busy. It is per-connection, not per-station: a fresh connection gets through two exchanges again while the wedged one is still being retransmitted into. What is left is below MicroPython - cyw43 driver logging, a monitor-mode capture, or a second access point, the last because the upstream reports of this signature include one that disappeared on a different AP.
- **The network intermittent is not debugpy: WiFi power save wedges one TCP connection (2026-08-13).** `20260813_wifi_powersave_tcp_stall.md`, with the reproduction kept beside it in `planning/repro/20260813_wifi_powersave/`. A 120-line MicroPython script replaying the session's wire shape - request 205 bytes, request 206, request 83 - wedges the same way with no `debugpy`, no `settrace` and no mpremote in it, so nothing this repo owns is involved. Power-save mode is the variable: cyw43 PM1 (`network.WLAN.PM_POWERSAVE`) wedges 6 of 6 raw trials and fails the HIL stepping scenario 13 of 13, the bench default PM2 is clean 120 of 120 raw and 25 of 25 through the production stack, and `PM_NONE` is clean. **The board is neither asleep nor dead while it is wedged**: it answers 91 pings across two 20 s stalls with no gap over a second, and RSTs a SYN to a closed port in 21-25 ms, three times - so frames arrive, IP and TCP answer them synchronously, and one established pcb has stopped taking data. On the wire the third request sits at exactly `rcv_nxt` with 5989 bytes of window open and is retransmitted five times over 15 s, unacknowledged, with no RST, so lwIP matched it to the pcb and dropped it; the device-side probe shows the wait loop running at 82 iterations a second with every `recv` timing out and `incoming=0`, so nothing ever reaches the socket. Two claims in `20260811_network_dap_tx_stall.md` are superseded: the `settimeout` alternation is ruled out from both directions, and the advertised window staying at 5989 shows nothing, because lwIP only announces a window increase once it reaches `LWIP_MIN(TCP_WND/2, mss)` - 800 bytes here. Not yet answered, and the first step of an upstream report: `tcp_in.c:422-435` is the only silent drop of in-window data on an established pcb, and modlwip's only refusing callback is `_lwip_tcp_recv_unaccepted` (`modlwip.c:560`), which `accept()` replaces - `lwip_socket_accept` does drop the lwIP lock between taking the pcb off the queue and installing the real callback (lines 1160-1182), which is visible as the 108 ms before every session's first reply, but that refusal should clear on the next 250 ms `tcp_fasttmr`. Documented rather than worked around: `docs/debugging.md` now tells users to turn power save off for a debug session, and `tests/hil/board_boot.py` deliberately keeps the default, because a bench configured to avoid the intermittent stops being able to catch it.
- **A console nobody reads stops the board, and `mpremote debug` was holding one (2026-08-13).** `20260813_console_backpressure.md`. The minimal reproduction the TX-stall note prescribed was built, and its hypothesis is dead: a device script with no `debugpy` in it, pumping at 80 Hz with the same `settimeout(0.001)`/`settimeout(None)` alternation and answering the same two-send-then-one-send shape, answers 5 of 5 in 6-16 ms, and removing the alternation changes nothing. That note also retired one hypothesis it had not tested: it read `extmod/modlwip.c:92-95` as saying the lwIP concurrency hooks are empty on this port, but those are the fallback definitions and stm32 defines its own at `ports/stm32/mphalport.h:66`, raising the interrupt priority to PENDSV. What the reproduction found instead is a defect in this repo's own command. A USB CDC console a host holds open and never reads back-pressures into the device: the tty stops accepting once its 4 kB buffer fills, the board's 1 kB transmit buffer fills behind it, and `usbd_cdc_tx_always` then waits for room for **each byte**, up to 500 ms - in `__WFI()`, so interrupts keep running and lwIP keeps acknowledging while Python makes no progress at all. Every symptom the TX-stall note could not place follows from that: a send the application believes it made, a receive window that never reopens although the request was handled, ICMP answered throughout, the board's own 30 s timeout printed at the end. `do_debug` started a reader for the primary connection only when the session also mounted a directory, so a `serial_dap` session and a `--dap-log` session both sat in `_stay_attached` holding the console with nothing reading it. Measured on the bench: a `--dap-log` session stopped answering after 5124 bytes of program output, twice at the same byte - 4096 + 1024, the two buffers - against 9150 with the console closed. `_stay_attached` now drains it for as long as it waits and prints what it reads, which is also the only place a `serial_dap` session's program output can appear, and is what the unix path already does with the child it owns; a `repl_dap` session passes None, since its channel reader owns that port. Pinned over a pty by a real `debugpy` session printing 100 kB, which without the drain stops after 20020 bytes. **This does not close the 2026-08-11 intermittent**: the HIL harness drains the console for the whole of every run, so the scenario that failed 4 of 5 that day is not exposed to this, and it passes 5/5 today with the flag off and 5/5 with it on. Worth carrying anyway - the risk row prescribed `--dap-log` as the instrument for catching that intermittent, and until this fix `--dap-log` was the one flag that could cause a stall of its own.
- **The network intermittent is a TCP send that never leaves the board (2026-08-11).** `20260811_network_dap_tx_stall.md`. The risk row had it as an intermittent seen twice and green six times after; it failed 4 of 5 runs that day, and a hand-driven client reproduced it 3 of 3, so the row's rate was wrong and the hunt was worth reopening. Instrumenting every step on the device shows each one succeeding: the board receives `setBreakpoints`, handles it, and `sock.send()` returns the full byte count for the response. `tcpdump` shows nothing leaving the board after that - no data segment, no retransmission - for the whole 20 s until the client sends FIN, which the board ACKs. Its receive window shrinks by the request and never reopens although Python read those bytes, it answers ICMP without loss throughout, and it prints its own 30 s `wait_for_client` timeout at the end. Input path alive, output path dead, and nothing in `debugpy` is in a position to cause that. The STORY-6.7 work is ruled out by measurement rather than by argument: reverting the one device-side change that touches the network path and reinstalling, the same test still failed 4 of 5. What the session bought beyond the diagnosis is method - the instrument that mattered was not the DAP log the row had prescribed but `tcpdump` beside a print in `_send_all`, because the question "did the bytes leave" cannot be answered from either end's own account of itself. Next step is a minimal reproduction with no `debugpy` in it; the one thing the pump does that ordinary socket code does not is alternate `settimeout(0.001)` and `settimeout(None)` about 80 times a second, and that is what to vary first.
- **STORY-6.7 DONE (2026-08-11): a board with one UART and no network reaches a breakpoint, and the failure only hardware could show was that the debugger did not give the console back.** `s6.7_single-uart-dap.md`, `20260811_hil_PYBD_SF6_no-dap-device.md`. DAP is framed into the stream mpremote is already holding, the way `mount` interleaves its filesystem RPC: `0x18` marker, fs-hook codes 1..13 untouched, `CMD_DAP`/`CMD_DAP_ACK`/`CMD_DAP_EOF` at 14..16, 128-byte payloads under a 192-byte credit window, and `0x18` in program output escaped to `0x18 0x18` rather than distinguished by content. The framing exists twice on purpose - `repl_mux.py` on the device, `repl_dap.py` on the host, because mpremote cannot depend on a `micropython-lib` package being installed - and `TestCrossImplementation` feeds identical bytes to both readers so the copies cannot drift. The host suite proves the wire; what it structurally cannot prove is the installation, because the unix port has no `os.dupterm`, and that is where the defect was. **A session that ended before any client attached left the framing wrapper in `dupterm` slot 1, and nothing short of a power cycle cleared it** - the wrapper answers every later REPL in framed bytes, and while it is installed stm32 has detached the interface from the REPL, so Ctrl-C arrives as data and cannot break out. The diagnostic signature is a plain REPL probe reading back `18 0f 02 00 02 00`: the mux acking the two bytes of `\r\n` it just consumed. Two causes, both in `StreamTransport` and both invisible without a board. `_peer_gone` armed itself on traffic alone, which is right for a dedicated interface nobody holds until the client connects and wrong for a stream the host has been driving the board over since before the program started; it now also arms on the channel having been held when the transport was built. And a `recv` given no timeout sat in one `poll` forever, so the liveness line - which no poll can wait on - was never re-read; it now waits in 100 ms slices. `_accept_and_initialize` retries a partial `initialize` rather than reading it as a client that sent something else, and tears the session down when the channel closes instead of building one on a message that never arrived. Since a stream with no `isconnected` cannot end its own session at all, `_repl_dap_stream()` refuses one up front rather than taking the console and never giving it back. Measured: the REPL answers 0.06 s after mpremote exits, against never. The bench arrangement is now a file (`tests/hil/board_boot_single_vcp.py`) rather than whoever set the bench up last, since it is decided at boot and cannot be reached from a test. Throughput 81.5-81.7 kB/s over five runs, against 81.7-108.8 kB/s for the same payload over a dedicated interface. Eight scenarios on the bench, five for this channel and the three STORY-6.1 single-CDC ones that share the arrangement.
- **STORY-6.1 DONE (2026-08-10): the criterion that had been waiting on a board since 2026-08-08 was answerable on the bench board all along.** Criterion 2 asks what a user sees when a target is configured for serial DAP and the board cannot provide it, and it was recorded as needing an ESP32_GENERIC that is offline here. D7 dissolved that: `ports/stm32/main.c` brings up a single VCP unless `boot.py` has already called `pyb.usb_mode()`, so the PYBD-SF6W booted without that line *is* the board - measured as `usb_mode VCP+MSC` beside `second_cdc: True`, built for two interfaces and booted with one, which is the same absence as a board that could never have had one as far as `_board_dap_stream()` can tell. Two halves of the criterion's wording are superseded rather than unmet: the board is a PYBD and not an ESP32_GENERIC, and D5 makes the answer a refusal rather than a fall-through, since `_detect_dap_stream()` raises before `debugpy.listen()` and the device never reports an endpoint it cannot serve. `tests/test_s6_1_no_second_cdc.py` drives all four routes to that refusal under the real interpreter - no `pyb`, a `pyb` without `usb_mode`, a `usb_mode` reporting one interface, a `USB_VCP(1)` that refuses to construct - and proves nothing was bound by connecting a socket rather than by reading output, with a TCP control so "nothing listened" cannot be a broken measurement; restoring the pre-D5 fall-through fails ten of the twelve. On hardware (`20260810_hil_PYBD_SF6_no-dap-device.md`) the refusal takes 0.57 s, never mentions the `dap_device` path the target names, and leaves a board that still answers a REPL prompt. **The first bench run failed on the harness, not the board, and the defect was general:** `read_until` matched `MPDBG-READY ` anywhere in a line, and mpremote's report that no handshake arrived quotes that marker, so a test asserting "no handshake" was handed mpremote's statement that there was none - and the shared HIL fixtures would have died parsing JSON out of an error message instead of failing with what they meant. Handshake call sites now match at line start, as `mpdebug_handshake.read_handshake` does; pinned by contract tests including the trap itself, and mutation-checked. `test_s5_3_unix_flow.py`'s private copies of the two helpers are gone, since both copies carried it. The single-CDC scenarios need the opposite bench arrangement from the serial-DAP ones, so a results record is now named for its arrangement and neither can silently replace the other. **Running both arrangements found a second defect of the same class, in the record writer.** It recorded only scenarios that reached their call phase, so three that broke in setup were left out of the record altogether - the arrangements are mutually exclusive by construction, and the single-CDC gate failed rather than skipping when the bench was in the other one. Recording setup errors then exposed the other half: a run refused at the attribution gate has results but no facts, so it wrote a record naming an unknown board, and that record is untracked, which dirties the tree, which refuses the next run, which writes it again. Both directions are pinned by `tests/test_hil_record_writer.py`, which needs no board. A full two-interface run is 19 passed, 3 skipped.
- **D7 (2026-08-10, user correction): the second CDC is not the no-network path, and the guide had been selling it as one.** `20260810_single-uart-dap-framing.md`. No board enumerates a second CDC by default: `ports/stm32/main.c:688-695` brings up a single VCP unless `boot.py` has already called `pyb.usb_mode()`, and `MICROPY_HW_USB_CDC_NUM (2)` only raises the ceiling. The bench PYBD reports `2xVCP+MSC` because a `boot.py` from an earlier session sets it - the HIL records have carried that line as provenance since 2026-08-08 while nothing stated it as a precondition and `docs/debugging.md` never told a reader to do it. So the serial path costs a user a `boot.py` edit and a re-enumeration *before* anything else, which is more device-side setup than the network path, not less; presenting it as the low-setup option was wrong in the guide and wrong in the framing of STORY-6.1. The real answer for boards without a network is to interleave DAP with the REPL on the one UART the way `mpremote mount` interleaves its filesystem RPC - `0x18` marker, host echo, `kbd_intr(-1)` for the transaction (`transport_serial.py:551-600`) - which is now STORY-6.7, with the two things fs-hook does not solve named in the story rather than discovered later: DAP requests arrive from the host at arbitrary times where every fs-hook exchange is device-initiated, and the program's own stdout shares a stream whose marker byte is unescaped. In the medium term USB NCM (`micropython/micropython#16459`, open) makes the network transport a cable path with no WiFi and nothing in this repo to change, which is the likelier route for most boards; `#19103` (runtime USB class selection, open) would remove the `boot.py`-and-reboot precondition above. Neither is a dependency and nothing is scheduled against them. Q12 is undisturbed - `second_cdc` still measures the build's maximum, which is still what a firmware manifest should advertise - but a `true` there means "could enumerate a second interface", never "has one ready", and the guide now says which of the three instruments answers which question. Re-reading the two documents to fix the framing found them already drifted in the way Q12's own finding predicted: `docs/debugging.md`'s `requires` vocabulary and `docs/firmware.md`'s variant table and glossary all predated `second_cdc`, so the published `requires` list omitted a key a user could legally write. Those are the fifth and sixth copies of a vocabulary the Q12 work had found four of and held together with tests, and nothing held these; `tests/test_docs_capabilities.py` now does, matching whole tokens rather than substrings - a first cut using `in` passed a mutation that renamed the key, since every capability name is a prefix of a plausible neighbour. `tests/hil/conftest.py`'s serial skip also names the boot precondition now, so an operator reading it does not go looking for a firmware problem.
- **Q12 closed (2026-08-10): the manifest says "this build has a second CDC" with a boolean `second_cdc`, and the blocker that deferred the question was about the wrong instrument.** `20260810_q12_second_cdc.md`. The question was parked on STORY-3.2's republish because `pyb.usb_mode()` reports the boot-time choice rather than the build's maximum. That is true of `usb_mode()` and false of the board: `pyb.USB_VCP(id)` constructs iff `id < MICROPY_HW_USB_CDC_NUM`, a compile-time constant, so the build maximum has a direct probe and the three questions here - what the build has, what this boot enumerated, what a host is holding - can each answer differently on one run. The boot script probes it and merges it into `MPDBG-READY`, rather than `debugpy.get_capabilities()`, because USB topology is not the debug server's concern and `debugpy` is bound for upstream. Boolean rather than a count, since `MICROPY_HW_USB_CDC_NUM` exists only in `ports/stm32` and `ports/mimxrt`. CI derives it with a new `verify_capabilities.py --report NAME=MACRO>=N`: `--expect` cannot express a threshold on a count whose `1` is truthy while meaning the capability's absence. Proved in both directions, which is what rules out a probe reading the wrong signal - stubs whose `usb_mode()` contradicts their own interface count make a `usb_mode`-based probe fail two cases in opposite directions - and on the PYBD, where the handshake and an independent REPL probe both report true. Two things the work exposed rather than added: the capability vocabulary has four hand-copies that could not import each other and nothing held them together until a release job, and mpremote's `requires` had no test for which USB-shaped caps are legal in it. All four `firmware.toml` entries carry `false`, honest for the published artifacts; the PYBD's becomes true at the republish, from CI rather than by hand. Corrected 2026-08-10: the note said the macro "exists only in `ports/stm32` and `ports/mimxrt`", which is true of where it is defined and misleading about where it is read - only `ports/stm32/usb.c` reads it, and `PHYBOARD_RT1170` defines it as `(2)` on a port that takes `CFG_TUD_CDC` from a different, boolean macro. So the `--report` rule would claim a second interface for a mimxrt build that enumerates one. Latent, since no variant is mimxrt, and recorded in `verify_capabilities.py` where someone adding a build job meets it.
- **`make lint-submodules` now runs the two tree-content jobs it was only naming (2026-08-10).** Knowing which jobs gate a branch is half the row; reproducing them is the other half, and two were reachable on this machine. micropython's `code_formatting` runs uncrustify over C and then checks that nothing moved - reproduced with the CI-pinned `micropython-uncrustify==1.0.0.post1`, in about 9s, currently clean on the integration tree. micropython-lib's `build_packages` is three steps, all three independently load-bearing: the manifest walk catches a bad `require()`, the index build catches a package source that will not compile, and the example compile catches neither. All three pass for the composed tree, in about 15s. Two deviations from CI are deliberate. CI clones micropython master for mpy-cross and `manifestfile.py`; this uses the submodule, which is that master plus the composed PRs. And the examples compile to a temp file rather than next to their source, because CI's throwaway checkout can afford 76 stray `.mpy` files in the tree and a developer's cannot. `package_tests` is knowingly not reproduced: its test list is fixed and does not name `python-ecosys/debugpy`. Both new targets were mutation-checked, including the case that separates this from CI's own `git diff --exit-code` - a working tree that is dirty but correctly formatted still passes, because the check compares the diff either side of the formatter rather than against `HEAD`.
- **Which gating jobs a submodule PR would run, and which have never run for the branch, is now computed rather than described (2026-08-10).** The risk row said to read the PR's check list by hand because a branch's own `.github/workflows/` is not what CI uses. Both sets are derivable: a `pull_request` event runs the workflows from the merge with the base, so upstream/master's set is what a PR faces, while a push to the fork runs the pushed commit's, so the branch's set is what has executed. `tools/check_submodule_ci.py` (`make check-submodule-ci`) takes the difference and finds four registered branches carrying one - `pdb_support`, which is PR #8767's own head, and `local_names_implementation` face six jobs each, among them `ports_unix`'s `standard_error_terse` and `standard_error_none`, which are test-suite jobs rather than exotic ports; `mpremote_close_lost_device` faces three; and `mpy-debugpy-foundations` still faces the `codespell` job the row was originally written about. The check applies each workflow's `paths` filter to the branch's actual diff, using GitHub's glob rules rather than `fnmatch`, so `debug_board_flags` (three board headers) is correctly told it faces ten jobs and not fifty-three; a filter the translator does not implement counts as gating, since over-reporting costs a look and under-reporting is the failure being prevented. It fetches before answering, for the same reason the pin check does, and `make integrate` prints the report non-fatally at the end - the rebuild moves the integration branch and leaves the feature branches, which are the ones that face CI, untouched. The manual instruction is not retired: this reasons about job names and cannot tell whether one passed. 17 host tests over throwaway repositories, every behaviour mutation-checked.
- **The one CI failure since the gates were fixed was a test waiting for less than it asserted (2026-08-10).** Run `31356566744` failed only `test_malformed_content_length_does_not_kill_forwarding`, which sends a frame with a bad `Content-Length` followed by a good one and checks that both reach the device. Its read loop ended as soon as `HELLO` - the first frame - appeared, and then asserted the second one against whatever had been read by that point; on a loaded runner the two writes arrived in separate reads and the assertion ran on the first. The product could not have caused it: `PumpingProxy._pump` forwards each chunk with `dst.sendall` *before* handing it to the parser and swallows parser faults, so a malformed header cannot wedge forwarding. Both copies of the test - the serial bridge's and the `DapProxy` mirror it was cloned from - now loop on the conjunction of both markers, and the pty side's bare `os.read` on a blocking master is bounded by the caller's deadline through `select`, so a frame that never arrives ends the loop and fails the assertion instead of parking the run. Mutation-checked in both files by dropping the second write: each fails in about five seconds naming the frame that never came.
- **The harness's waiting helpers can no longer report success without having found anything, and following the risk row's instruction found a second instance (2026-08-10).** `339192a` fixed the reported symptom in `wait_for_msg` and left the mechanism: the `count` parameter and its `len(rcv_messages) >= count` return survived, so a call naming neither an event nor a response still answered true for having received anything at all. No call site had ever passed `count`, so the parameter is gone and a criterion-free wait raises. The row's standing instruction - watch for a helper whose success value comes from a default parameter rather than from what it found - then turned up `set_breakpoints(..., wait=False)` returning a bare `True`; its three asserting call sites all pass `wait=True` and were sound, but the fire-and-forget mode now returns None so an `assert` on it cannot pass on a request nobody confirmed. Both are pinned by `tests/test_harness_wait_helpers.py`, which models the two ways messages reach a helper rather than one: `wait_for_msg` pumps the socket itself, one `run_single` per poll, so its fake drips messages through that call and a timeout stays reachable, while `take_msg` never polls and reads what the server's reader thread already appended. Getting that wrong is why the first `take_msg` case failed, and a fake carrying only the pumped mode would have tested the wrong helper.
- **A HIL run now records the tree it ran against, and refuses to start from one it could not name (2026-08-10).** The risk row was written after a run's three failures were read as a device regression and turned out to be a working-tree draft of a test that was never committed. `tests/hil/tree_state.py` puts the top-repo commit and both submodule pins in the results record and fails the run when anything is uncommitted, with `MPY_DEBUG_HIL_ALLOW_DIRTY=1` to override and list the dirt in the record instead. The gate runs after the device gate, so a host-only `make test` on a dirty checkout still skips; the record is written from the state captured at the start of the run, since writing the record is itself what dirties the tree; and one `git status --porcelain` at the top level covers the submodules, which is where the debugpy sources copied onto the board come from. It is tested on the host over throwaway repositories rather than only on the bench, for the reason the wired-up-but-never-ran row gives.
...
- **Every upstream PR this project feeds has a red lint job, in both
  ...
  **Two standing consequences.** A submodule edit is not verified by
  `make lint`; `make lint-submodules` reproduces the tree-content CI jobs and
...
- **D3 (2026-07-05) — Serial DAP is board-dependent** — first-class only on multi-CDC
  ...
  - *Correction (2026-08-10, user):* the decision holds and the ordering it sets
    was never in doubt, but "multi-CDC boards" understates the precondition and
    had been read here as though the second interface were there for the taking.
    A build with `MICROPY_HW_USB_CDC_NUM (2)` still boots one VCP unless
    `boot.py` calls `pyb.usb_mode("2xVCP+MSC")` (`ports/stm32/main.c:688-695`),
    which no board in this set ships. Serial DAP therefore costs a user *more*
    device-side setup than the network path, not less, and must never be
    presented as the easier option. `20260810_single-uart-dap-framing.md`.
...
- **D9 (2026-08-20, user) — DAP over a dedicated serial interface is removed, not just
  demoted.** D8 stopped recommending it; this deletes it. Gone: `serial_dap.py` and its
  bridge, `_maybe_start_serial_dap`/`_serial_dap_lost`, the `dap_device` target key, the
  device-side probe for a board's own DAP interface, the `serial_dap` and `second_cdc`
  capabilities and their manifest column and CI rules, and the `MICROPY_HW_USB_CDC_NUM (2)`
  patch on the PYBD_SF6. Two routes remain and the docs lead with them: a board as it ships
  shares the REPL's stream, and a board worth compiling for gets a network. What stays is
  the named-stream mechanism underneath - a `dap_stream` path this runtime can open, which
  the unix port uses and `--dap-repl` rides. The bench collapses with it: one boot script,
  one arrangement, no swap-and-power-cycle between runs, and a results record named by date
  and board alone. 43 tests went with the path and 450 remain.
- **D8 (2026-08-19, user) — USB networking, not a second CDC, is the advice for a custom
  build.** `network.USBD_NCM` is merged upstream (`extmod/network_usbd_ncm.c`, master tip
  `1c3c201149`, with the TinyUSB integration in `f86bd73c35`), which is the route D7 named
...
- **D7 (2026-08-10, user) — the no-network path is in-band framing on the REPL
  ...
  and is not the answer for users without a network — it needs a `boot.py` edit
  and a re-enumeration on top of the right build. The answer for one-UART boards
  is to interleave DAP with the REPL the way `mpremote mount` interleaves its
  filesystem RPC (STORY-6.7). Separately, `micropython/micropython#16459` (USB
  NCM) makes the already-written network transport work over the cable with no
  WiFi, which is the likelier medium-term route once it merges; nothing is
  scheduled against it. `20260810_single-uart-dap-framing.md`.
...
- **Q12 → a boolean `second_cdc` capability, probed by constructing
  `pyb.USB_VCP(1)`.** `20260810_q12_second_cdc.md`. The question was recorded as
  waiting on STORY-3.2's republish because "`pyb.usb_mode()` reports the
  boot-time choice rather than the build's maximum" - true of `usb_mode()`, and
  not true of the board. There are three questions, and hardware measurement
  showed all three can disagree on one run: `pyb.USB_VCP(id)` constructs iff
  `id < MICROPY_HW_USB_CDC_NUM`, a compile-time constant, so it reads the build
  maximum directly; `usb_mode()` reads what this boot enumerated; `isconnected()`
  reads whether a host is holding the interface. The manifest wants the first, so
  the question was decidable now. Boolean rather than a count because
  `MICROPY_HW_USB_CDC_NUM` exists only in `ports/stm32` and `ports/mimxrt` -
  rp2/esp32 express the same thing as TinyUSB's `CFG_TUD_CDC` and ESP32_GENERIC
  has no USB device peripheral - so a count is a field three of the four current
  variants cannot fill in. Probed by the boot script rather than by
  `debugpy.get_capabilities()`, since USB topology is not the debug server's
  concern and `debugpy` is bound for upstream micropython-lib; `_run()` copies
  the capability dict before adding to it, because with a session live
  `get_capabilities()` returns the session's own. Still distinct from
  `serial_dap`, which reports the channel one session took and never appears in
  the manifest: a board with the interface debugged over TCP reports
  `second_cdc` true and `serial_dap` false in the same handshake. No capability
  guard change was needed - `capcheck` only errors on claimed-`True`/probed-not-
  `True`, so a wrong `false` costs a variant and a wrong `true` is a hard error.
  CI derives it with a new `verify_capabilities.py --report NAME=MACRO>=N`,
  because `--expect` cannot express a threshold on a count whose `1` is truthy
  and means the capability's absence; an undefined macro reports `false` (the
  real answer for rp2/esp32/unix) while a non-integer value stays an error, and
  reports are evaluated after the `--expect` failures so a failing build says
  what actually failed. Evidence in both directions: `tests/test_q12_second_cdc.py`
  drives the shipped boot script under the real unix firmware with `pyb` stubs
  whose `usb_mode()` contradicts their own interface count, so a probe reading
  `usb_mode` gets every case backwards - mutating the probe that way fails three
  of five, in opposite directions; and the PYBD reports `second_cdc` true in the
  handshake and in an independent REPL probe. The vocabulary now has four
  hand-copies (`firmware.py`, `gen_manifest.py`, `write_fragment.py`,
  mpremote's `mpdebug_config.py`), none able to import another, so tests hold
  them to each other - a key one accepts and another rejects would otherwise
  surface in a release job after every artifact is built. `second_cdc` is a
  legal `requires` for an mpremote target and `serial_dap` still is not, for the
  same build-versus-run reason.
...
- **Q3 → ship v1 network-only.** Single-UART serial DAP is pushed back, not dropped: the
  ...
  *Amended 2026-08-10 (D7):* the follow-up now has a story (STORY-6.7) and a
  technique to copy rather than design from the spike's sketch — `mpremote
  mount`'s in-band marker framing. What "pushed back" costs was also
  understated: with the second CDC needing a `boot.py` edit and a reboot, v1
  network-only means every user without a network is waiting on STORY-6.7 or on
  USB NCM, not on a board purchase. `20260810_single-uart-dap-framing.md`.
...
- **STORY-6.1 — Serial device transport**
  - **DONE 2026-08-10 (criterion 2).** The last open criterion is met, on the
    board that was here all along: an stm32 that never calls `pyb.usb_mode()`
    enumerates one CDC interface, so the PYBD-SF6W booted without that line is
    a board with no dedicated DAP interface. A target configured for serial DAP
    is refused in 0.57 s, naming the board's missing interface and not the
    `dap_device` node the host never opens. See the ticket.
...
- **STORY-6.7 — DAP interleaved with the REPL on a single UART**
  - **DONE 2026-08-11** — all five criteria on the bench PYBD_SF6 booted
    single-VCP; see Status, `s6.7_single-uart-dap.md`, and
    `20260811_hil_PYBD_SF6_no-dap-device.md`. Raised 2026-08-10 from a user
    correction; see D7 and `20260810_single-uart-dap-framing.md`.
  - **Amended 2026-08-10:** the "route program output through the DAP `output`
    event" alternative is not available from Python on the device, so the
    choice is narrower than the note that raised this story recorded.
    `print()`'s `file` default is a ROM pointer at `mp_sys_stdout_obj`
    (`py/modbuiltins.c:393`), `sys.stdout` is not assignable (the mutable `sys`
    attributes are `path`, `ps1`, `ps2`, `tracebacklimit` - `py/modsys.c:298-320`),
    and `os.dupterm` adds a destination rather than replacing one
    (`mp_hal_stdout_tx_strn`, `ports/stm32/mphalport.c:82-105`). Rebinding
    `print` in the debuggee's globals does not cover tracebacks or a direct
    `sys.stdout.write`. So either the marker byte is escaped in the device's
    stdout path, or the divert is made in C - a port-level stdout hook, or a
  ...
  - description: the no-network answer for boards with one UART, which is most
    ...
    with the REPL on one stream (`0x18` marker, host echo, `kbd_intr(-1)` for
    the transaction), and that is the technique to copy. Two things it does not
  ...
  - acceptance criteria:
    - [x] a board with one UART reaches a breakpoint over that UART, with no
      network configured and no second interface enumerated.
    - [x] a program that prints the marker byte does not desynchronise the
      link, and the test writes it deliberately.
    - [x] the host distinguishes program output from DAP frames without a
      heuristic on content.
    - [x] throughput measured on hardware and recorded, as STORY-6.1's was.
    - [x] whichever of escaping / diverting stdout is chosen, the trade-off it
  ...
  - not scheduled against `micropython/micropython#16459` (USB NCM): if that
    lands first, an NCM board takes the existing network path unchanged and
    this story narrows to boards that get neither.
...
11. **STORY-6.1** (needs 2.2,5.4), **STORY-6.2** (needs 5.4,EPIC-4) — parallel.
    ...
    hardware attached. **STORY-6.2 DONE 2026-08-08. STORY-6.1 DONE 2026-08-10:
    its positive path was proven on the same board on 2026-08-08, criteria 4
    and 3 on 2026-08-10, and criterion 2 the same day once D7 showed the board
    with no second CDC was the bench board booted without a `pyb.usb_mode()`
    call.**
...
16. **STORY-8.3** (needs 8.1,6.4), **STORY-8.4** (needs EPIC-5,6,3) — parallel.
    **STORY-8.4 DONE 2026-08-09**, and closing it closed STORY-6.3's second
    ...
    moves the branch.
    What remains not autonomously reachable: STORY-4.4 is unreachable by
    design, and the two pushes that would put either fold in front of reviewers
    are the user's call. **STORY-6.1's criteria 4, 3 and 2 were all reachable
    and were closed on 2026-08-10**, criterion 2 last, once D7 showed that the
    board with no second CDC it had been waiting for is the bench board booted
    without a `pyb.usb_mode()` call - so "needs a board that is not here" had
    been true only of the criterion's wording, not of the question it asks.
    Criterion 4 closed once "handled" was read as what a DAP client can
    actually be told rather than as ampremote's reconnect, and criterion 3 by
    giving a channel that cannot reach EOF something else to report the host
    leaving with. **Q14 and Q11 were both reachable
    ...
    #51 untouched. **Q12 closed the same way on 2026-08-10, and was the third
    of three.** It was recorded as waiting on STORY-3.2's republish "rather than
    on anything decidable now"; the blocker it named was true of `pyb.usb_mode()`
    and not of the board, since `pyb.USB_VCP(1)` reads the build maximum
    directly. No question is open now, which retires the sequence's premise
    rather than completing it. **STORY-6.6
...
| ~~Serial DAP framing on single-UART boards unproven, and it is the *only* no-network path for most boards (reframed 2026-08-10, D7)~~ PROVEN 2026-08-11 on the bench PYBD_SF6 booted single-VCP (STORY-6.7): a breakpoint, an adversarial marker in program output, and 81.5-81.7 kB/s. The row is kept for what proving it cost - the framing worked first time and the *release* of the REPL stream did not, on a path no host test can reach - and because one board is not the board set: the mechanism is `dupterm` slot 1 holding the REPL as a Python object, which is stm32-on-the-legacy-USB-stack and nothing else today. Original: | network transport stays mainline (D3); the framing layer is now STORY-6.7 with a technique to copy (`mpremote mount`'s `0x18` in-band marker) rather than a blank spike. The row was previously mitigated by "multi-CDC boards have the serial path" - they do not, without a `boot.py` edit and a re-enumeration no board ships, so a one-UART board and a PYBD out of the box are in the same position. USB NCM **is merged as of 2026-08-19** (`extmod/network_usbd_ncm.c`, upstream master `1c3c201149`) and retires the row for boards that can take it, by making the existing network transport a cable path - see D8. Still not a dependency: it is off by default, no upstream port enables it yet, and it needs a TinyUSB port, which the bench PYBD is not |
...
| ~~`assert wait_for_msg(...)` asserted nothing for the life of the suite - the helper returned `len(rcv_messages) >= count` with `count` defaulting to 0, i.e. true even on timeout, across nine call sites (found 2026-08-06)~~ **CLOSED 2026-08-10, and the 2026-08-06 fix had left the trap reachable.** `339192a` made the matched-message path return the message, but kept the `count` parameter and its `len(rcv_messages) >= count` return, so a call naming neither an event nor a response still answered true for having received anything at all. No call site had ever passed `count`; the parameter is gone and a criterion-free wait is now a `ValueError` | the instruction the row carried is discharged rather than retired, because following it found a second instance: `set_breakpoints(..., wait=False)` returned a bare `True`. Its three asserting call sites all pass `wait=True` and were sound, but an `assert` on the fire-and-forget mode would have passed on a request nobody confirmed, so that mode now returns None too. Both are pinned by `tests/test_harness_wait_helpers.py`, whose fake server carries the two delivery modes the helpers assume - `wait_for_msg` pumps the socket itself one `run_single` per poll, `take_msg` reads what the reader thread already appended - because a fake that only models one of them tests the wrong helper. Keep watching for the shape: a helper whose success value is computed from a default parameter rather than from what it found |
...
| ~~A background HIL run whose output is kept but whose tree is not proves nothing, and one was read as a regression before that was noticed: a run on 2026-08-09 at 12:29 failed 3 of 11 network scenarios, and its surviving traceback shows a revision of `test_hil_a_long_pause_does_not_drop_the_link` that expected a fresh connection to the endpoint to *succeed*. No such revision was ever committed - the file has one commit, and that line number falls inside the docstring there - so the run was against a discarded working-tree draft, and all three failures are unattributable rather than flaky. The committed test asserts the refusal, which is what the server does by design~~ **ENFORCED 2026-08-10 - the pairing is done by the suite, not by the operator.** `tests/hil/tree_state.py` refuses to start a run from a dirty tree and puts the top-repo commit and both submodule pins at the top of the results record. `MPY_DEBUG_HIL_ALLOW_DIRTY=1` overrides the refusal and lists the uncommitted paths in the record instead, so a bench that has to run a change before committing it still produces something attributable | the reasoning the row was written for stands: a failing scenario whose expectation cannot be traced to a commit is not evidence about the device, and that applies to an all-pass record too - it is what one run of one tree saw. Three ordering details are deliberate. The gate sits *after* the device gate, so a host-only `make test` on a dirty tree still skips rather than fails. The record is written from the state captured at the start of the run, because the run dirties the tree by writing the record. And `git status --porcelain` at the top level is what reports a touched submodule, which is where the debugpy sources this suite copies onto the board actually live. Exercised on the host rather than only on the bench (`tests/test_hil_tree_state.py`, six cases over throwaway repositories), because a gate that only the bench runs is the wired-up-but-never-ran shape two rows above |
| **REPRODUCED WITH NO DEBUGPY IN IT AT cyw43 PM1, 2026-08-13, but the sightings in this row are all PM2 - see `20260813_wifi_powersave_tcp_stall.md`.** Every failure this row records happened on the bench default (`PM_PERFORMANCE`, 0xa11142, which is PM2 and still a sleeping mode); the boot script sets no `pm`. At PM1 (`PM_POWERSAVE`) the same wire shape wedges 6 of 6 with a 120-line MicroPython script and no debugpy, and fails the HIL stepping scenario 13 of 13, while PM2 is clean 120 of 120 raw and 25 of 25 through the command. So PM1 is an amplifier that makes the fault reproducible, not the condition for it, and **that the rare PM2 failure is the same fault is not measured**: the capture and the device probe both come from PM1 runs, and 2026-08-11's capture points the other way on direction (outbound dead there, inbound in every PM1 wedge). What would settle it is a long soak of the debugpy-free harness at PM2. While wedged the board answers pings and RSTs a closed port in ~20 ms, so it is one pcb refusing data, not a board asleep. The row stays open because the bench runs PM2, where the fault is rare rather than absent, and because which layer drops the segment is not yet pinned. Not papered over: the bench keeps the default so it can still catch it, and the advice to turn power save off is in the user docs. **SEEN AGAIN 2026-08-13, once in about twenty runs of the scenario, and the step this row prescribed found a different defect instead - see `20260813_console_backpressure.md`.** The sighting is `test_hil_stepping_advances_the_program[next]` again, same signature, in a full-suite run on the committed tree; the board's captured stdout ends at `[DAP] Debug session ready - all other messages will be handled in trace function`, so the device finished `initialize` and then answered nothing. It reproduced once on the next standalone run and then passed 5 of 5 alone, and 2 of 2 immediately after a full suite, so "after a full suite" is not the trigger either. **PARKED 2026-08-15 - retried once by signature, recorded as retried, and not otherwise worked around; see the Status entry of that date.** **2026-08-14: the losses are below lwIP, and the whole investigation is written up for handover in `20260814_wifi_stall_investigation_handover.md`.** With lwIP's counters compiled in, an outbound-connection wedge shows `tcp.drop` unmoved and every error counter at zero while 22 unicast pings in the same window are answered, and the capture shows eight retransmissions over 34 s drawing no ACK. Not debugpy, not lwIP's TCP, not the accept path. Open below that, and open on whether the PM2 sightings share the mechanism. **STILL OPEN 2026-08-13.** The minimal reproduction ruled out the `settimeout` alternation (5/5 answered, 6-16 ms), and the scenario now passes 5/5 with `--dap-log` off and 5/5 with it on, so nothing about this sighting is currently reproducible. What the hunt did find is that a console a host holds open and never reads stops the board in `print` while lwIP keeps acknowledging - which is this row's exact signature - and that `mpremote debug` was holding one on every stay-attached path without a mount. The HIL harness drains the console throughout, so that defect does not account for these runs; a hand-driven client that did not drain would be. Also carried: this row prescribed `--dap-log` as the instrument, and until 2026-08-13 `--dap-log` was the one flag that could cause a stall of its own. **LOCALISED 2026-08-11 to a TCP send that never leaves the board, and it is not intermittent at the rate this row records - see `20260811_network_dap_tx_stall.md`.** It failed 4 of 5 runs that day, and a hand-driven client reproduced it 3 times out of 3. `sock.send()` returns the full count for the `setBreakpoints` response and `tcpdump` shows nothing on the wire from the board afterwards - no data segment and no retransmission - for the whole 20 s, while the board ACKs the request, answers ICMP without loss, and prints its own 30 s `wait_for_client` timeout at the end. So the input path lives and the output path is dead for that pcb, and every step from `recv` through the handler to `send` reports success. The STORY-6.7 work is ruled out by reverting the one device-side change on the network path and measuring again: 4 of 5 still failed. The next step is a minimal reproduction with no `debugpy` in it, varying the `settimeout(0.001)`/`settimeout(None)` alternation the pump does 80 times a second, which is the one thing it does that ordinary socket code does not. Original: the HIL network suite has an intermittent that survived a bounded hunt (observed 2026-08-10, twice in one ~5 minute window): `test_hil_stepping_advances_the_program[next]` failed with `no response to setBreakpoints (seq 2) within 20s; received since: []`, which means the board answered `initialize` on that same connection and then said nothing for 20 s. It failed inside a full-suite run and again on the next standalone run, then passed on the six runs after it, including three back-to-back and one begun immediately after `mpy-dev cycle pybd` finished enumerating - so the power cycle the first failure followed is ruled out as the trigger, and so is "first session after boot", since many network scenarios passed before it in the failing run. Nothing about it is specific to `next`; `stepIn`, the parametrisation beside it, passed both times | unresolved, and deliberately not papered over by raising the 20 s timeout: a board that answers one request and not the next is not slow, and a longer wait would convert a visible intermittent into a slower green run. The next occurrence should be caught with the session's `--dap-log` on, which records what the device saw rather than what the client did not; the record `20260810_hil_PYBD_SF6.md` is written from pytest's own reports, so a failing run cannot be mistaken for a missing one. **INSTRUMENTED 2026-08-10, timeout unchanged.** Two instruments, and which one is always on follows from what a timeout can and cannot say. `no response ... received since: []` reads the same whether the board is wedged, has rebooted, or raised inside the request handler, and only the last of those leaves a trace - a MicroPython traceback on the board's own stdout, which a network session's `DeviceOutput` is already reading for the length of the test. So `Recorder.silence_note` attaches that capture to every DAP silence unconditionally: it costs nothing, needs no flag set in advance, and is the only place that answer can be. `--dap-log` is the second instrument and is *not* on by default - it works by interposing a host-side proxy and reporting the proxy's endpoint, so under it a run no longer reports the board's own address, which is exactly what three of these scenarios assert; those carry `hil_direct_endpoint` and stay unproxied whatever `MPY_DEBUG_HIL_DAP_LOG` says. Verified both ways on the bench: 11 passed unproxied and 11 with the flag, which wrote 8 transcripts, one per unmarked scenario. The caveat that follows is the one to carry into any hunt - the transport under the flag is not the transport that produced the intermittent, so failing to reproduce it there is not evidence. `silence_note` is exercised on the host rather than only when the bench next breaks (`tests/test_hil_silence_note.py`), mutation-checked by dropping the note from the failure text: three of its four cases fail, the fourth being the negative control for a session that has nothing to report |
| A test that waits for one marker and then asserts two is a race the local suite hides: `test_malformed_content_length_does_not_kill_forwarding` waited until the *first* frame appeared and then asserted the second one, which is the frame the test exists to check. It passed locally for the life of the story and reddened CI run `31356566744` (2026-08-10) on a loaded runner, where the two writes landed in separate reads. The product is exonerated by ordering, not by the pass: `PumpingProxy._pump` calls `dst.sendall(chunk)` before `parser.feed(chunk)` and catches parser exceptions, so no header can stop forwarding | fixed in both copies - the loop condition is now the conjunction of everything asserted below it, so the wait ends when the claim is true or when the deadline says it never became true. Mutation-checked by deleting the second `sendall`: each fails in about five seconds naming the missing frame. The other half was a hang risk rather than a flake: the pty reader was a bare `os.read` on a blocking master, so "the bytes never came" would have parked the run instead of failing it; `_read_master` now takes the caller's deadline and returns `b""` through `select`. Generalise both: a deadline loop must test the full postcondition, and every read inside one must be bounded by the same deadline |
| ~~A submodule's CI is not fully described by the workflows in its own tree: micropython-lib's `codespell` job exists only on upstream master, so a branch that predates it runs the check on every PR while showing no sign of it locally, and the job that reports as **`build`** in a PR's check list is the commit-message check from `commit_formatting.yml`. `make lint-submodules` reproduces only the two ruff commands and would have said "clean" for both (found 2026-08-09)~~ **MEASURED 2026-08-10 - the gap is derivable, so it is derived rather than described.** `tools/check_submodule_ci.py` (`make check-submodule-ci`) computes both sets from git: what a PR faces is upstream/master's `pull_request` jobs, because that event runs the workflows from the merge with the base; what has actually executed is the branch's own, because a push to the fork runs the pushed commit's. Four registered branches face a job that has never run for them - `pdb_support` (PR #8767) and `local_names_implementation` six each, including `ports_unix`'s `standard_error_terse`/`standard_error_none` and two qemu jobs; `mpremote_close_lost_device` three; and `mpy-debugpy-foundations` the very `codespell` job this row was written about, still, a year of upstream later | the manual instruction stands and is not replaced: `gh api repos/<upstream>/commits/<sha>/check-runs` on the PR itself is the final word, because this check reasons about job *names* and cannot know whether one passed. What it removes is the part that was guesswork - noticing that a job exists at all. Three details are deliberate. It fetches `upstream/master` first, for the reason the pin check does. It applies each workflow's `paths` filter to the branch's own diff, so a `tools/mpremote` branch is not told about esp32 jobs, with GitHub's glob semantics rather than `fnmatch` (`*` stops at a separator, `**` crosses it) and a pattern it cannot translate counting as gating rather than dropping out. And it labels each branch with the PR it reaches or `no PR`, which is the distinction the stranded-commits row's fourth correction is about. `make integrate` prints the report non-fatally afterwards, because a rebuild moves the integration branch and leaves the feature branches - the ones that face CI - exactly where they were. Tested on the host over throwaway repositories (`tests/test_submodule_ci_check.py`, 17 cases), each behaviour mutation-checked. **REPRODUCED 2026-08-10** - naming a gating job is not running it, so the two that are reachable on this machine now run from `make lint-submodules`: `make check-c-format` (micropython's `code_formatting`, uncrustify at the CI pin, ~9s) and `make check-lib-packages` (micropython-lib's `build_packages`, manifests + index + examples, ~15s). Both pass on the composed tree; all three `build_packages` steps were shown to catch something the others miss. `check-c-format` compares the diff either side of the formatter rather than doing CI's `git diff --exit-code`, which is only meaningful on a clean checkout - a dirty but correctly formatted tree passes, and a hit is already fixed in the working tree. `package_tests` stays unreproduced on purpose: its test list is fixed and does not name `python-ecosys/debugpy`. Everything else upstream runs needs a toolchain this machine does not have, so for those the standing instruction is unchanged |
