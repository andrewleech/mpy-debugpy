# mp_debugpy — Roadmap to a user-friendly, upstream-mergeable debugging experience
...
- **EPIC-1 (foundations): DONE and verified.** STORY-1.1/1.2/1.3/1.4 implemented in the
  ...
  handshake, and an empty-`f_locals` `save_names` false-positive. Not yet committed.
- **STORY-3.1 (firmware manifest): DONE.** `firmware/firmware.toml` + `launcher/firmware.py`
  ...
  the pinned submodule SHA (only `unix_settrace_local_xx` matches `05b7818`); download URLs
  are placeholders pending a CI release step. Reinforces STORY-1.2 (probe, don't trust names).
...
- `frame._set_local` / `sys._set_local_var` do **not** exist in the pinned firmware
  (`05b7818`). setVariable on locals silently degrades to an error. Only global edits work.
  Local editing must not be promised in UX.
...
- `f_locals` is a fresh snapshot dict per access, not a live view. `f_back` and
  `f_globals` are live. Function parameters are omitted from `f_locals` in the SAVE_NAMES
  path; without SAVE_NAMES names are positional placeholders `local_N`.
- settrace is not on in the unix `standard` variant; debugging needs purpose-built firmware.
...
- **STORY-3.1 — Firmware manifest + hash-verified fetch**
  ...
  - description: Define a per-variant manifest (repo, SHA, flags, artifact sha256, URL).
    Publish binaries to GitHub Releases via CI; stop carrying binaries in-tree. Add a fetch
    that downloads and verifies against the manifest hash.
  - acceptance criteria:
...
- **STORY-3.2 — Reproducible docker build for firmware variants**
  - type: implementation
  - description: Dockerised build producing the two real variants with pinned flags
    (`settrace`, `settrace_save_names`). Host-uid/path convention per user CLAUDE.md.
  - acceptance criteria:
    - [ ] `settrace` and `settrace_save_names` build reproducibly in the container.
...
- **STORY-8.1 — Register `debug` command branch in mbm.toml**
  - type: implementation
  - description: Create the command as a branch off upstream master in
    `~/ampremote/micropython`, push to the fork, register in `mbm.toml` before
    `ampremote_rename` (which stays last), open a draft PR on the fork per policy.
  - acceptance criteria: [ ] branch builds into the `ampremote` integration via mbm; [ ]
    draft fork PR exists; [ ] `ampremote_rename` remains last.
  - dependencies: EPIC-5
...
- **STORY-8.2 — Upstream debugpy foundations**
  ...
  - acceptance criteria: [ ] PR(s) opened; [ ] lineage reconciled; [ ] the double-defined
    `MICROPY_PY_SYS_SETTRACE` build-glue issue addressed.
  - dependencies: EPIC-1
