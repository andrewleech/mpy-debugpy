...
- **STORY-6.4 — Device transport tests (hardware-in-loop)**
  - **Network half done on PYBD_SF6 (2026-08-08); OPEN for the serial half.**
    `tests/hil/` is the suite, opt-in behind `MPY_DEBUG_HIL_DEVICE`; it writes
    its own results record (`20260808_hil_PYBD_SF6.md`) from pytest's reports.
    No ESP32 on this bench, so the single-UART shape is still untested.
  - type: test
  ...
  - acceptance criteria: [ ] both device flows pass on real hardware (network:
    yes, 11 scenarios on PYBD_SF6; serial: no board wired for it yet);
    [x] results record board/port tested.
...
| The `extension-host` CI job (STORY-7.4) has never executed a run: its first attempt died in GitHub's 2026-08-06 Actions outage at the action-download step, the next two queued 15 minutes and were cancelled with no runner allocated. So its CI wiring is unverified, and the two steps local runs never exercise from cold - the VS Code download and the marketplace install of `ms-python.debugpy` - are exactly the ones most likely to differ there | re-run it once Actions recovers (`gh run rerun` on the push event; the release job is gated to `workflow_dispatch`, so a re-run cannot publish). The suite itself is green locally over 16 runs, two of them concurrent with the full pytest suite |
| Serial DAP throughput on a real board is unmeasured and could make the transport impractical: `StreamTransport.recv` reads one byte per iteration (MicroPython's `.read()`/`.readinto()` loop until full, and the pty stand-in genuinely blocks on the second internal read), measured 33.7 ms and 532 KB of allocation for 4095 bytes on desktop x86 | a device stream that returns short without blocking wants a chunked read, which is the correct code for hardware but cannot be validated on the stand-in - and getting it wrong reintroduces the deadlock class of `20260806_stopped-event-flake.md`. Measure a multi-KB `variables` response on a Pico W first thing when a bench is available; if it is seconds, the transport needs redesign, not tuning |
| ~~Busy-poll pause loop starves WiFi/housekeeping while paused on device~~ MEASURED 2026-08-08 on PYBD_SF6: not observable at 60 s | `test_hil_a_long_pause_does_not_drop_the_link` holds a session at a breakpoint with no DAP traffic for `MPY_DEBUG_HIL_PAUSE_SECONDS` (60 by default), then checks both the session's own socket and an unrelated connection attempt. Both answered. No tuning made; longer holds and other ports are unmeasured, so the scenario stays in the suite as the measurement rather than the row being deleted |
